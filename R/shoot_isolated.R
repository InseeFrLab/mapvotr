...
shoot_isolated <- function(contours,voronoi,min_address_shoot,var_bv = "id_brut_bv"){
  ...
  # little cast polygons are targeted
  ...
  for(i in pos_target){
    ...
    if (! identical(length(pos_voisins),0L) ) {
      ...
      if(sum(nb_addresses_neigh>min_address_shoot)>0){
        ...
        bv_voisins <- st_drop_geometry(contours_cast)[pos_voisins,var_bv,drop=T] %>% unique()
        if(length(bv_voisins)>1L){
          # if several neighbors (code_bv), big is beautiful 
          bv_replace <- contours_cast %>% 
            st_drop_geometry() %>% 
            filter(.data[[var_bv]] %in% bv_voisins ) %>% 
  ...
  # Union cast polygon using modified code_bv
  contours_res <- contours_res %>% select(.data[[var_bv]]) %>% 
    group_by(.data[[var_bv]]) %>% 
