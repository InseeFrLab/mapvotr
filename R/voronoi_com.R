...
voronoi_com <- function(sfelecteurs,com,var_bv){
  ...
  if (nrow(points_multiBV)>0L) logr::put(paste0(nrow(points_multiBV), " points affectes a des BV differents"))
  
  # Create Voronois polygons inside the city 'envelope"
  ...
  # voronoi <- voronoi %>% dplyr::mutate(id_voronoi=row.names(.)) # add id
  res <- sf::st_intersection(st_geometry(voronoi),st_geometry(com)) # avoid warnings this way
  res <- sf::st_as_sf(res) %>% dplyr::rename("geometry"="x")%>% dplyr::mutate(id_voronoi=row.names(.)) # add id
  # res <- sf::st_intersection(voronoi,com %>% dplyr::select(geometry)) # intersection with city contour
  return(res)
}
