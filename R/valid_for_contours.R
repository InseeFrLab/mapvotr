#' Check if the city can be divided into voting contours
#'
#' @description
...
#'
#' @param tab_address addresses of a single city
...
valid_for_contours <- function(tab_address, min_points_com, min_address_bv, var_bv1, var_nbaddress) {
  ...
  if (nb_row < min_points_com) {
    ...
    count_address_by_bv <- tab_address %>%
      dplyr::group_by(.data[[var_bv1]]) %>%
      dplyr::summarise(nb_address = sum(.data[[var_nbaddress]]))

    if (min(count_address_by_bv$nb_address) < min_address_bv) {
      logr::put(paste0("Not enough addresses by BV (threshold = ", min_address_bv, ")"))
      bv_problem <- count_address_by_bv %>%
        dplyr::filter(nb_address < min_address_bv) %>%
        logr::put()
